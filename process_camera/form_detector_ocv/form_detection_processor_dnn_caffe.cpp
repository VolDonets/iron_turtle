...
void FormDetectionProcessorDNN::processRecognition() {
    recognitionProcessThread = std::thread([this]() {
        while (true) {
            ...
            cv::Mat resizedFrame;
            cv::resize(currentFrame, resizedFrame, cv::Size(256, 256));
            cv::cvtColor(resizedFrame, resizedFrame, cv::COLOR_BGRA2BGR);
            cv::Mat inputBlob = cv::dnn::blobFromImage(resizedFrame, 1, cv::Size(256, 256), cv::Scalar(104, 177, 123), false);
            dnn_model.setInput(inputBlob, "data");
            cv::Mat detection = dnn_model.forward("softmax8");
