...
void increase_object_rectangle() {
    double newScale = scalePursuit + PURSUIT_SCALE_STEP;
    if (newScale > 1)
        return;
    int step_x = (newScale * WIDTH) - widthPursuitRect;
    int step_y = (newScale * HEIGHT) - heightPursuitRect;
...
void decrease_object_rectangle() {
    ...
    if (newScale < 0)
        return;
    int steps_x = widthPursuitRect - (newScale * WIDTH);
    int steps_y = heightPursuitRect - (newScale * HEIGHT);

    xPursuitRect = xPursuitRect + (steps_x / 2);
    yPursuitRect = yPursuitRect + (steps_y / 2);
    widthPursuitRect = widthPursuitRect - steps_x;
    heightPursuitRect = heightPursuitRect - steps_y;
    scalePursuit = newScale;
}

void move_lefter_object_rectangle() {
    xPursuitRect = ((xPursuitRect - PURSUIT_STEP_X) >= 0) ? (xPursuitRect - PURSUIT_STEP_X) : xPursuitRect;
}
